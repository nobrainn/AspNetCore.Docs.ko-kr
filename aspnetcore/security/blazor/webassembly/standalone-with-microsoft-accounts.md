...
Blazor인증을 위해 [AAD (Azure Active Directory)를](/azure/active-directory/develop/quickstart-register-app#register-a-new-application-using-the-azure-portal) 사용 하는 Microsoft 계정을 사용 하는 weasembom독립형 앱을 만들려면:
...
1. [AAD 테 넌 트 및 웹 응용 프로그램 만들기](/azure/active-directory/develop/v2-overview)
   ...
   3. **리디렉션 uri** 드롭다운을 **웹**으로 유지 하 고의 리디렉션 uri를 제공 `https://localhost:5001/authentication/login-callback` 합니다.<br>
   4 \. **Permissions**  >  **Grant admin 동의 하도록 요구 to openid connect and offline_access permissions 확인란을** 사용 하지 않도록 설정 합니다.<br>
   5. **등록**을 선택합니다.

   **인증**  >  **플랫폼 구성**  >  **웹**에서:

   1\. 의 **리디렉션 URI** 가 있는지 확인 `https://localhost:5001/authentication/login-callback` 합니다.<br>
...
* 앱을 올바르게 구성 하는 경우 독립 실행형 앱과 동일한 방법을 사용 하 여 Microsoft Api에 대 한 액세스 토큰을 요청 Blazor 합니다. 자세한 내용은 [빠른 시작: 웹 api를 노출 하도록 응용 프로그램 구성](/azure/active-directory/develop/quickstart-configure-app-expose-web-apis)을 참조 하세요.
