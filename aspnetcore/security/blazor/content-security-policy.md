...
응용 프로그램에 CSP를 적용 하기 위해 개발자는 하나 이상의 헤더 또는 태그에 여러 CSP 콘텐츠 보안 *지시문* 을 지정 합니다 `Content-Security-Policy` `<meta>` .

페이지를 로드 하는 동안 브라우저에서 정책을 평가 합니다. 브라우저는 페이지의 소스를 검사 하 여 콘텐츠 보안 지시문의 요구 사항을 충족 하는지 확인 합니다. 리소스에 대해 정책 지시문이 충족 되지 않으면 브라우저에서 리소스를 로드 하지 않습니다. 예를 들어 타사 스크립트를 허용 하지 않는 정책을 고려해 보세요. 페이지에 `<script>` 특성에서 타사 원본을 사용 하는 태그가 포함 되어 있으면 `src` 브라우저에서 스크립트가 로드 되지 않도록 방지 합니다.

CSP는 Chrome, Edge, Firefox, Opera 및 Safari를 비롯 한 대부분의 최신 데스크톱 및 모바일 브라우저에서 지원 됩니다. 앱에는 CSP가 권장 됩니다 Blazor .
...
* 응용 프로그램을 로컬로 실행 하는 동안 브라우저의 개발자 도구 콘솔에 액세스 합니다. 브라우저는 CSP 헤더 또는 태그가 있을 때 차단 된 스크립트에 대 한 해시를 계산 하 고 표시 합니다 `meta` .
* 브라우저에서 제공 하는 해시를 원본에 복사 `script-src` 합니다. 각 해시 주위에 작은따옴표를 사용 합니다.
...
태그를 사용 `<meta>` 하 여 정책을 적용 합니다.

* 특성의 값을 `http-equiv` 로 설정 `Content-Security-Policy` 합니다.
* 특성 값에 지시문을 추가 `content` 합니다. 지시문을 세미콜론 ()으로 구분 `;` 합니다.
* 항상 `meta` 콘텐츠에 태그를 추가 `<head>` 합니다.

다음 섹션에는 Blazor weasembmbambmbmbmbommbmboma 및 Server에 대 한 Blazor 이러한 예제는의 각 릴리스에 대 한이 문서에서 버전 관리 됩니다 Blazor . 릴리스에 적절 한 버전을 사용 하려면이 웹 페이지의 **버전** 드롭다운 선택기를 사용 하 여 문서 버전을 선택 합니다.

### <a name="blazor-webassembly"></a>Blazor WebAssembly

`<head>` *Wwwroot/index.html* 호스트 페이지의 내용에서 [정책 지시문](#policy-directives) 섹션에 설명 된 지시문을 적용 합니다.
...
이전 지시문을 지원 하려면 라는 헤더를 사용 `Content-Security-Policy` 합니다. 지시문 문자열은 헤더의 값입니다.
...
정책 지시문을 적용 하지 않고 일정 시간 동안 정책을 테스트 하려면 `<meta>` `http-equiv` 헤더 기반 정책의 태그 특성 또는 헤더 이름을로 설정 `Content-Security-Policy-Report-Only` 합니다. 오류 보고서는 지정 된 URL에 JSON 문서로 전송 됩니다. 자세한 내용은 [MDN 웹 문서: 내용-보안-정책-보고서 전용](https://developer.mozilla.org/docs/Web/HTTP/Headers/Content-Security-Policy-Report-Only)을 참조 하세요.
...
`report-uri`는 더 이상 사용 하지 않는 것이 좋지만 `report-to` 모든 주요 브라우저에서를 지원할 때까지 두 지시문을 모두 사용 해야 합니다. `report-uri`에 대 한 지원은 `report-uri` 브라우저에서 언제 *든* 지 삭제 될 수 있으므로 독점적으로 사용 하지 마세요. `report-uri`가 완전히 지원 되는 경우 정책에서에 대 한 지원을 제거 `report-to` 합니다. 채택을 추적 하려면 `report-to` [사용 가능: 보고 대상](https://caniuse.com/#feat=mdn-http_headers_csp_content-security-policy_report-to)을 참조 하세요.
