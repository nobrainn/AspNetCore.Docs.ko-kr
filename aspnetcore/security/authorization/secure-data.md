...
<span data-ttu-id="25a14-125">다음 그림은 연락처의 관리자 세부 정보 보기를 보여 줍니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-125">The following image shows the managers details view of a contact:</span></span>
...
## <a name="the-starter-and-completed-app"></a><span data-ttu-id="25a14-145">시작 및 완료 된 앱</span><span class="sxs-lookup"><span data-stu-id="25a14-145">The starter and completed app</span></span>
...
<span data-ttu-id="25a14-181">`ContactIsOwnerAuthorizationHandler` *권한 부여* 폴더에 클래스를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-181">Create a `ContactIsOwnerAuthorizationHandler` class in the *Authorization* folder.</span></span> <span data-ttu-id="25a14-182">는 `ContactIsOwnerAuthorizationHandler` 리소스에 대해 작동 하는 사용자가 리소스를 소유 하는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-182">The `ContactIsOwnerAuthorizationHandler` verifies that the user acting on a resource owns the resource.</span></span>
...
<span data-ttu-id="25a14-207">이 섹션에서는 Razor Pages를 업데이트 하 고 작업 요구 사항 클래스를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-207">In this section, you update the Razor Pages and add an operations requirements class.</span></span>
...
* <span data-ttu-id="25a14-221">`Contact` 모델에 사용자 ID를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-221">Add the user ID to the `Contact` model.</span></span>
...
<span data-ttu-id="25a14-235">현재 UI에는 사용자가 수정할 수 없는 연락처에 대 한 편집 및 삭제 링크가 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-235">Currently, the UI shows edit and delete links for contacts the user can't modify.</span></span>
...
* <span data-ttu-id="25a14-248">사용자가 권한을 제거 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-248">Removing privileges from a user.</span></span> <span data-ttu-id="25a14-249">예를 들어 채팅 앱에서 사용자를 음소거 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-249">For example, muting a user in a chat app.</span></span>
...
* <span data-ttu-id="25a14-290">"연락처 관리자" 라는 Razor Pages 앱 만들기</span><span class="sxs-lookup"><span data-stu-id="25a14-290">Create a Razor Pages app named "ContactManager"</span></span>
...
<span data-ttu-id="25a14-319">다음 그림은 연락처의 관리자 세부 정보 보기를 보여 줍니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-319">The following image shows the managers details view of a contact:</span></span>
...
## <a name="the-starter-and-completed-app"></a><span data-ttu-id="25a14-339">시작 및 완료 된 앱</span><span class="sxs-lookup"><span data-stu-id="25a14-339">The starter and completed app</span></span>
...
<span data-ttu-id="25a14-375">*권한 부여* 폴더를 만들고 여기에 `ContactIsOwnerAuthorizationHandler` 클래스를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-375">Create an *Authorization* folder and create a `ContactIsOwnerAuthorizationHandler` class in it.</span></span> <span data-ttu-id="25a14-376">는 `ContactIsOwnerAuthorizationHandler` 리소스에 대해 작동 하는 사용자가 리소스를 소유 하는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-376">The `ContactIsOwnerAuthorizationHandler` verifies that the user acting on a resource owns the resource.</span></span>
...
<span data-ttu-id="25a14-401">이 섹션에서는 Razor Pages를 업데이트 하 고 작업 요구 사항 클래스를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-401">In this section, you update the Razor Pages and add an operations requirements class.</span></span>
...
<span data-ttu-id="25a14-408">위의 코드는</span><span class="sxs-lookup"><span data-stu-id="25a14-408">The preceding code:</span></span>

* <span data-ttu-id="25a14-409">권한 부여 `IAuthorizationService` 처리기에 액세스 하는 서비스를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-409">Adds the `IAuthorizationService` service to access to the authorization handlers.</span></span>
* <span data-ttu-id="25a14-410">Id `UserManager` 서비스를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-410">Adds the Identity `UserManager` service.</span></span>
* <span data-ttu-id="25a14-411">`ApplicationDbContext`를 추가합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-411">Add the `ApplicationDbContext`.</span></span>

### <a name="update-the-createmodel"></a><span data-ttu-id="25a14-412">CreateModel 업데이트</span><span class="sxs-lookup"><span data-stu-id="25a14-412">Update the CreateModel</span></span>

<span data-ttu-id="25a14-413">`DI_BasePageModel` 기본 클래스를 사용 하도록 create page model 생성자를 업데이트 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-413">Update the create page model constructor to use the `DI_BasePageModel` base class:</span></span>

[!code-csharp[](secure-data/samples/final2.1/Pages/Contacts/Create.cshtml.cs?name=snippetCtor)]

<span data-ttu-id="25a14-414">메서드를 `CreateModel.OnPostAsync` 다음으로 업데이트 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-414">Update the `CreateModel.OnPostAsync` method to:</span></span>

* <span data-ttu-id="25a14-415">`Contact` 모델에 사용자 ID를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-415">Add the user ID to the `Contact` model.</span></span>
* <span data-ttu-id="25a14-416">권한 부여 처리기를 호출 하 여 사용자에 게 연락처를 만들 수 있는 권한이 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-416">Call the authorization handler to verify the user has permission to create contacts.</span></span>

[!code-csharp[](secure-data/samples/final2.1/Pages/Contacts/Create.cshtml.cs?name=snippet_Create)]

### <a name="update-the-indexmodel"></a><span data-ttu-id="25a14-417">IndexModel 업데이트</span><span class="sxs-lookup"><span data-stu-id="25a14-417">Update the IndexModel</span></span>

<span data-ttu-id="25a14-418">승인 된 `OnGetAsync` 연락처만 일반 사용자에 게 표시 되도록 메서드를 업데이트 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-418">Update the `OnGetAsync` method so only approved contacts are shown to general users:</span></span>

[!code-csharp[](secure-data/samples/final2.1/Pages/Contacts/Index.cshtml.cs?name=snippet)]

### <a name="update-the-editmodel"></a><span data-ttu-id="25a14-419">EditModel 업데이트</span><span class="sxs-lookup"><span data-stu-id="25a14-419">Update the EditModel</span></span>

<span data-ttu-id="25a14-420">사용자가 연락처를 소유 하 고 있는지 확인 하기 위해 권한 부여 처리기를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-420">Add an authorization handler to verify the user owns the contact.</span></span> <span data-ttu-id="25a14-421">리소스 권한 부여의 유효성을 검사 하 `[Authorize]` 는 중이기 때문에 특성에 충분 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-421">Because resource authorization is being validated, the `[Authorize]` attribute is not enough.</span></span> <span data-ttu-id="25a14-422">특성이 평가 될 때 앱에서 리소스에 액세스할 수 없습니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-422">The app doesn't have access to the resource when attributes are evaluated.</span></span> <span data-ttu-id="25a14-423">리소스 기반 권한 부여는 필수적 이어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-423">Resource-based authorization must be imperative.</span></span> <span data-ttu-id="25a14-424">앱이 리소스에 대 한 액세스 권한이 있는 경우 페이지 모델에서 로드 하거나 처리기 자체 내에서 로드 하 여 검사를 수행 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-424">Checks must be performed once the app has access to the resource, either by loading it in the page model or by loading it within the handler itself.</span></span> <span data-ttu-id="25a14-425">리소스 키를 전달 하 여 리소스에 자주 액세스 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-425">You frequently access the resource by passing in the resource key.</span></span>

[!code-csharp[](secure-data/samples/final2.1/Pages/Contacts/Edit.cshtml.cs?name=snippet)]

### <a name="update-the-deletemodel"></a><span data-ttu-id="25a14-426">DeleteModel 업데이트</span><span class="sxs-lookup"><span data-stu-id="25a14-426">Update the DeleteModel</span></span>

<span data-ttu-id="25a14-427">권한 부여 처리기를 사용 하 여 사용자에 게 연락처에 대 한 삭제 권한이 있는지 확인 하기 위해 삭제 페이지 모델을 업데이트 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-427">Update the delete page model to use the authorization handler to verify the user has delete permission on the contact.</span></span>

[!code-csharp[](secure-data/samples/final2.1/Pages/Contacts/Delete.cshtml.cs?name=snippet)]

## <a name="inject-the-authorization-service-into-the-views"></a><span data-ttu-id="25a14-428">권한 부여 서비스를 뷰에 삽입 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-428">Inject the authorization service into the views</span></span>

<span data-ttu-id="25a14-429">현재 UI에는 사용자가 수정할 수 없는 연락처에 대 한 편집 및 삭제 링크가 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-429">Currently, the UI shows edit and delete links for contacts the user can't modify.</span></span>

<span data-ttu-id="25a14-430">모든 보기에서 사용할 수 있도록 *views/_ViewImports* 파일에 권한 부여 서비스를 삽입 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-430">Inject the authorization service in the *Views/_ViewImports.cshtml* file so it's available to all views:</span></span>

[!code-cshtml[](secure-data/samples/final2.1/Pages/_ViewImports.cshtml?highlight=6-99)]

<span data-ttu-id="25a14-431">위의 태그는 여러 `using` 문을 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-431">The preceding markup adds several `using` statements.</span></span>

<span data-ttu-id="25a14-432">적절 한 권한이 있는 사용자에 대해서만 렌더링 되도록 *Pages/Contacts/Index.* s e r v e r/ **삭제** **링크를 업데이트 합니다.**</span><span class="sxs-lookup"><span data-stu-id="25a14-432">Update the **Edit** and **Delete** links in *Pages/Contacts/Index.cshtml* so they're only rendered for users with the appropriate permissions:</span></span>

[!code-cshtml[](secure-data/samples/final2.1/Pages/Contacts/Index.cshtml?highlight=34-36,62-999)]

> [!WARNING]
> <span data-ttu-id="25a14-433">데이터를 변경할 수 있는 권한이 없는 사용자의 링크를 숨기면 앱이 보호 되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-433">Hiding links from users that don't have permission to change data doesn't secure the app.</span></span> <span data-ttu-id="25a14-434">링크를 숨기면 올바른 링크만 표시 하 여 앱을 보다 사용자에 게 친숙 하 게 만들 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-434">Hiding links makes the app more user-friendly by displaying only valid links.</span></span> <span data-ttu-id="25a14-435">사용자는 생성 된 Url을 해킹 하 여 자신이 소유 하지 않은 데이터에 대 한 편집 및 삭제 작업을 호출할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-435">Users can hack the generated URLs to invoke edit and delete operations on data they don't own.</span></span> <span data-ttu-id="25a14-436">Razor 페이지 또는 컨트롤러는 데이터를 보호 하기 위해 액세스 검사를 적용 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-436">The Razor Page or controller must enforce access checks to secure the data.</span></span>

### <a name="update-details"></a><span data-ttu-id="25a14-437">업데이트 세부 정보</span><span class="sxs-lookup"><span data-stu-id="25a14-437">Update Details</span></span>

<span data-ttu-id="25a14-438">관리자가 연락처를 승인 하거나 거부할 수 있도록 세부 정보 보기를 업데이트 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-438">Update the details view so managers can approve or reject contacts:</span></span>

[!code-cshtml[](secure-data/samples/final2.1/Pages/Contacts/Details.cshtml?name=snippet)]

<span data-ttu-id="25a14-439">세부 정보 페이지 모델을 업데이트 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-439">Update the details page model:</span></span>

[!code-csharp[](secure-data/samples/final2.1/Pages/Contacts/Details.cshtml.cs?name=snippet)]

## <a name="add-or-remove-a-user-to-a-role"></a><span data-ttu-id="25a14-440">역할에 사용자 추가 또는 제거</span><span class="sxs-lookup"><span data-stu-id="25a14-440">Add or remove a user to a role</span></span>

<span data-ttu-id="25a14-441">에 대 한 자세한 내용은 다음 [문제](https://github.com/dotnet/AspNetCore.Docs/issues/8502) 를 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="25a14-441">See [this issue](https://github.com/dotnet/AspNetCore.Docs/issues/8502) for information on:</span></span>

* <span data-ttu-id="25a14-442">사용자가 권한을 제거 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-442">Removing privileges from a user.</span></span> <span data-ttu-id="25a14-443">예를 들어 채팅 앱에서 사용자를 음소거 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-443">For example, muting a user in a chat app.</span></span>
* <span data-ttu-id="25a14-444">사용자에 게 권한 추가</span><span class="sxs-lookup"><span data-stu-id="25a14-444">Adding privileges to a user.</span></span>

## <a name="test-the-completed-app"></a><span data-ttu-id="25a14-445">완성 된 앱 테스트</span><span class="sxs-lookup"><span data-stu-id="25a14-445">Test the completed app</span></span>

<span data-ttu-id="25a14-446">시드 된 사용자 계정에 대 한 암호를 아직 설정 하지 않은 경우 [비밀 관리자 도구](xref:security/app-secrets#secret-manager) 를 사용 하 여 암호를 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-446">If you haven't already set a password for seeded user accounts, use the [Secret Manager tool](xref:security/app-secrets#secret-manager) to set a password:</span></span>

* <span data-ttu-id="25a14-447">강력한 암호 선택: 8 개 이상의 문자 및 하나 이상의 대문자, 숫자 및 기호를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-447">Choose a strong password: Use eight or more characters and at least one upper-case character, number, and symbol.</span></span> <span data-ttu-id="25a14-448">예를 들어 `Passw0rd!` 은 강력한 암호 요구 사항을 충족 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-448">For example, `Passw0rd!` meets the strong password requirements.</span></span>
* <span data-ttu-id="25a14-449">프로젝트의 폴더에서 다음 명령을 실행 합니다. 여기서 `<PW>` 은 암호입니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-449">Execute the following command from the project's folder, where `<PW>` is the password:</span></span>

  ```dotnetcli
  dotnet user-secrets set SeedUserPW <PW>
  ```

* <span data-ttu-id="25a14-450">데이터베이스 삭제 및 업데이트</span><span class="sxs-lookup"><span data-stu-id="25a14-450">Drop and update the Database</span></span>
...
* <span data-ttu-id="25a14-451">응용 프로그램을 다시 시작 하 여 데이터베이스를 시드해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-451">Restart the app to seed the database.</span></span>

<span data-ttu-id="25a14-452">완성 된 앱을 테스트 하는 쉬운 방법은 세 가지 브라우저 (또는 incognito/InPrivate 세션)를 시작 하는 것입니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-452">An easy way to test the completed app is to launch three different browsers (or incognito/InPrivate sessions).</span></span> <span data-ttu-id="25a14-453">한 브라우저에서 새 사용자를 등록 합니다 (예: `test@example.com`).</span><span class="sxs-lookup"><span data-stu-id="25a14-453">In one browser, register a new user (for example, `test@example.com`).</span></span> <span data-ttu-id="25a14-454">다른 사용자로 각 브라우저에 로그인 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-454">Sign in to each browser with a different user.</span></span> <span data-ttu-id="25a14-455">다음 작업을 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-455">Verify the following operations:</span></span>

* <span data-ttu-id="25a14-456">등록 된 사용자는 승인 된 모든 연락처 데이터를 볼 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-456">Registered users can view all of the approved contact data.</span></span>
* <span data-ttu-id="25a14-457">등록 된 사용자는 자신의 데이터를 편집/삭제할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-457">Registered users can edit/delete their own data.</span></span>
* <span data-ttu-id="25a14-458">관리자는 연락처 데이터를 승인/거부할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-458">Managers can approve/reject contact data.</span></span> <span data-ttu-id="25a14-459">보기 `Details` 는 **승인** 및 **거부** 단추를 표시 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-459">The `Details` view shows **Approve** and **Reject** buttons.</span></span>
* <span data-ttu-id="25a14-460">관리자는 모든 데이터를 승인/거부 하 고 편집/삭제할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-460">Administrators can approve/reject and edit/delete all data.</span></span>

| <span data-ttu-id="25a14-461">사용자</span><span class="sxs-lookup"><span data-stu-id="25a14-461">User</span></span>                | <span data-ttu-id="25a14-462">앱에서 시드</span><span class="sxs-lookup"><span data-stu-id="25a14-462">Seeded by the app</span></span> | <span data-ttu-id="25a14-463">옵션</span><span class="sxs-lookup"><span data-stu-id="25a14-463">Options</span></span>                                  |
| ------------------- | :---------------: | ---------------------------------------- |
| test@example.com    | <span data-ttu-id="25a14-464">아니요</span><span class="sxs-lookup"><span data-stu-id="25a14-464">No</span></span>                | <span data-ttu-id="25a14-465">자신의 데이터를 편집/삭제 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-465">Edit/delete the own data.</span></span>                |
| manager@contoso.com | <span data-ttu-id="25a14-466">예</span><span class="sxs-lookup"><span data-stu-id="25a14-466">Yes</span></span>               | <span data-ttu-id="25a14-467">자신의 데이터를 승인/거부 하 고 편집/삭제 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-467">Approve/reject and edit/delete own data.</span></span> |
| admin@contoso.com   | <span data-ttu-id="25a14-468">예</span><span class="sxs-lookup"><span data-stu-id="25a14-468">Yes</span></span>               | <span data-ttu-id="25a14-469">모든 데이터를 승인/거부 하 고 편집/삭제 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-469">Approve/reject and edit/delete all data.</span></span> |

<span data-ttu-id="25a14-470">관리자의 브라우저에서 연락처를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-470">Create a contact in the administrator's browser.</span></span> <span data-ttu-id="25a14-471">관리자 연락처에서 삭제 및 편집에 대 한 URL을 복사 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-471">Copy the URL for delete and edit from the administrator contact.</span></span> <span data-ttu-id="25a14-472">이러한 링크를 테스트 사용자의 브라우저에 붙여넣어 테스트 사용자가 이러한 작업을 수행할 수 없는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-472">Paste these links into the test user's browser to verify the test user can't perform these operations.</span></span>

## <a name="create-the-starter-app"></a><span data-ttu-id="25a14-473">시작 앱 만들기</span><span class="sxs-lookup"><span data-stu-id="25a14-473">Create the starter app</span></span>

* <span data-ttu-id="25a14-474">"연락처 Razor 관리자" 라는 페이지 앱 만들기</span><span class="sxs-lookup"><span data-stu-id="25a14-474">Create a Razor Pages app named "ContactManager"</span></span>
  * <span data-ttu-id="25a14-475">**개별 사용자 계정을**사용 하 여 앱을 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-475">Create the app with **Individual User Accounts**.</span></span>
  * <span data-ttu-id="25a14-476">네임 스페이스는 샘플에서 사용 되는 네임 스페이스와 일치 하도록 "연락처 관리자"로 이름을 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-476">Name it "ContactManager" so the namespace matches the namespace used in the sample.</span></span>
  * <span data-ttu-id="25a14-477">`-uld`SQLite 대신 LocalDB를 지정 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-477">`-uld` specifies LocalDB instead of SQLite</span></span>

  ```dotnetcli
  dotnet new webapp -o ContactManager -au Individual -uld
  ```

* <span data-ttu-id="25a14-478">*모델/연락처를 추가 합니다. cs*:</span><span class="sxs-lookup"><span data-stu-id="25a14-478">Add *Models/Contact.cs*:</span></span>

  [!code-csharp[](secure-data/samples/starter2.1/Models/Contact.cs?name=snippet1)]

* <span data-ttu-id="25a14-479">모델을 `Contact` 스 캐 폴드 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-479">Scaffold the `Contact` model.</span></span>
* <span data-ttu-id="25a14-480">초기 마이그레이션을 만들고 데이터베이스를 업데이트 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-480">Create initial migration and update the database:</span></span>
...
* <span data-ttu-id="25a14-481">*Pages/_Layout. cshtml* 파일에서 지 각 **관리자** 앵커를 업데이트 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-481">Update the **ContactManager** anchor in the *Pages/_Layout.cshtml* file:</span></span>

  ```cshtml
  <a asp-page="/Contacts/Index" class="navbar-brand">ContactManager</a>
  ```

* <span data-ttu-id="25a14-482">연락처를 만들고, 편집 하 고, 삭제 하 여 앱을 테스트 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-482">Test the app by creating, editing, and deleting a contact</span></span>

### <a name="seed-the-database"></a><span data-ttu-id="25a14-483">데이터베이스 시드</span><span class="sxs-lookup"><span data-stu-id="25a14-483">Seed the database</span></span>

<span data-ttu-id="25a14-484">*데이터* 폴더에 [SeedData](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/security/authorization/secure-data/samples/starter2.1/Data/SeedData.cs) 클래스를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-484">Add the [SeedData](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/security/authorization/secure-data/samples/starter2.1/Data/SeedData.cs) class to the *Data* folder.</span></span>

<span data-ttu-id="25a14-485">다음 `SeedData.Initialize` 에서 `Main`호출:</span><span class="sxs-lookup"><span data-stu-id="25a14-485">Call `SeedData.Initialize` from `Main`:</span></span>

[!code-csharp[](secure-data/samples/starter2.1/Program.cs?name=snippet)]

<span data-ttu-id="25a14-486">응용 프로그램이 데이터베이스를 시드 하는지 테스트 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-486">Test that the app seeded the database.</span></span> <span data-ttu-id="25a14-487">Contact DB에 행이 있는 경우 시드 방법이 실행 되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-487">If there are any rows in the contact DB, the seed method doesn't run.</span></span>

::: moniker-end

<a name="secure-data-add-resources-label"></a>

### <a name="additional-resources"></a><span data-ttu-id="25a14-488">추가 자료</span><span class="sxs-lookup"><span data-stu-id="25a14-488">Additional resources</span></span>

* [<span data-ttu-id="25a14-489">Azure App Service에서 .NET Core 및 SQL Database 웹앱 빌드</span><span class="sxs-lookup"><span data-stu-id="25a14-489">Build a .NET Core and SQL Database web app in Azure App Service</span></span>](/azure/app-service/app-service-web-tutorial-dotnetcore-sqldb)
* <span data-ttu-id="25a14-490">[권한 부여 랩을 ASP.NET Core](https://github.com/blowdart/AspNetAuthorizationWorkshop)합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-490">[ASP.NET Core Authorization Lab](https://github.com/blowdart/AspNetAuthorizationWorkshop).</span></span> <span data-ttu-id="25a14-491">이 랩에서는이 자습서에서 소개 하는 보안 기능에 대해 자세히 설명 합니다.</span><span class="sxs-lookup"><span data-stu-id="25a14-491">This lab goes into more detail on the security features introduced in this tutorial.</span></span>
* <xref:security/authorization/introduction>
* [<span data-ttu-id="25a14-492">사용자 지정 정책 기반 권한 부여</span><span class="sxs-lookup"><span data-stu-id="25a14-492">Custom policy-based authorization</span></span>](xref:security/authorization/policies)
