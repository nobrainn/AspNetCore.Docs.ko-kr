...
인터페이스 `IAuthorizationPolicyProvider` 에는 다음과 같은 세 가지 api가 포함 되어 있습니다.
...
권한 부여 정책은 이름으로 식별 됩니다. 앞에서 `MinimumAgeAuthorizeAttribute` 설명한 사용자 지정은 인수를 해당 권한 부여 정책을 검색 하는 데 사용할 수 있는 문자열에 매핑해야 합니다. 에서 `AuthorizeAttribute` 파생 시켜 속성을 래핑하 `Age` `AuthorizeAttribute.Policy` 는 방법으로이 작업을 수행할 수 있습니다.
...
사용자 지정 `IAuthorizationPolicyProvider` 구현을 사용 하는 경우 ASP.NET Core는 인스턴스를 하나만 사용 한다는 점에 `IAuthorizationPolicyProvider`유의 하세요. 사용자 지정 공급자가 사용할 모든 정책 이름에 대해 권한 부여 정책을 제공할 수 없는 경우 백업 공급자에 게이를 연기 해야 합니다. 
...
위에 표시 `IAuthorizationPolicyProvider` 된 예제 구현은 해당 생성자에서 백업 정책 공급자 `DefaultAuthorizationPolicyProvider` 를 만들어를 사용 하도록 업데이트할 수 있습니다 (정책 이름이 ' 이상 보존 기간 ' + 연령의 예상 패턴과 일치 하지 않는 경우에 사용 됨).
...
그런 다음 null `GetPolicyAsync` 을 반환 하는 대신를 `BackupPolicyProvider` 사용 하도록 메서드를 업데이트할 수 있습니다.
...
사용자 지정 `IAuthorizationPolicyProvider` 은 선택적으로 `GetFallbackPolicyAsync` 를 구현 하 여 정책을 [조합할](/dotnet/api/microsoft.aspnetcore.authorization.authorizationpolicy.combine) 때 및 정책을 지정 하지 않을 때 사용 되는 정책을 제공할 수 있습니다. 에서 `GetFallbackPolicyAsync` null이 아닌 정책을 반환 하는 경우 요청에 정책을 지정 하지 않은 경우 권한 부여 미들웨어에서 반환 된 정책을 사용 합니다.
