...
이러한 유형의 작업에는 여러 반환 형식 및 경로가 있으므로 [`[ProducesResponseType]`](xref:Microsoft.AspNetCore.Mvc.ProducesResponseTypeAttribute) 특성을 자유롭게 사용 해야 합니다. 이 특성은 [Swagger](xref:tutorials/web-api-help-pages-using-swagger)와 같은 도구에서 생성한 웹 API 도움말 페이지에 대한 설명이 포함된 응답 세부 정보를 생성합니다. `[ProducesResponseType]`은 작업에서 반환한 알려진 형식 및 HTTP 상태 코드을 나타냅니다.
...
ASP.NET Core 2.1 [`[ApiController]`](xref:Microsoft.AspNetCore.Mvc.ApiControllerAttribute) 이상의 특성이 적용 되는 경우 모델 유효성 검사 오류는 400 상태 코드를 생성 합니다. 자세한 정보는 [자동 HTTP 400 응답](xref:web-api/index#automatic-http-400-responses)을 참조하세요.
...
* 특성 [`[ProducesResponseType]`](xref:Microsoft.AspNetCore.Mvc.ProducesResponseTypeAttribute) 의 `Type` 속성을 제외할 수 있습니다. 예를 들어 `[ProducesResponseType(200, Type = typeof(Product))]`은 `[ProducesResponseType(200)]`으로 단순화됩니다. 작업의 예상 반환 형식은 대신 `ActionResult<T>`의 `T`에서 유추됩니다.
