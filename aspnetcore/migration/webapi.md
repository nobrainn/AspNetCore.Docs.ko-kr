...
*Global.asax.cs*에서 다음을 `WebApiConfig.Register`호출 합니다.

[!code-csharp[](webapi/sample/ProductsApp/Global.asax.cs?highlight=14)]

클래스 `WebApiConfig` 는 *App_Start* 폴더에 있으며 정적 `Register` 메서드를 포함 합니다.

[!code-csharp[](webapi/sample/ProductsApp/App_Start/WebApiConfig.cs)]

이 클래스는 실제로 프로젝트에서 사용 되지 않지만 [특성 라우팅을](/aspnet/web-api/overview/web-api-routing-and-actions/attribute-routing-in-web-api-2)구성 합니다. 또한 ASP.NET Web API에서 사용 하는 라우팅 테이블을 구성 합니다. 이 경우 ASP.NET 4.x Web API는 Url과 일치 하는 `/api/{controller}/{id}`url을 선택 사항으로 `{id}` 예상 합니다.

*ProductsApp* 프로젝트는 하나의 컨트롤러를 포함 합니다. 컨트롤러는에서 `ApiController` 상속 되며 다음 두 가지 동작을 포함 합니다.

[!code-csharp[](webapi/sample/ProductsApp/Controllers/ProductsController.cs?highlight=28,33)]

에서 `Product` `ProductsController` 사용 하는 모델은 간단한 클래스입니다.
...
* **파일** > **새로 만들기** > **Project**프로젝트 > **기타 프로젝트 형식** > **Visual Studio 솔루션**으로 이동 합니다. **비어 있는 솔루션**을 선택 하 고 솔루션의 이름을 *WebAPIMigration*로 선택 합니다. **확인** 단추를 클릭합니다.
...
ASP.NET Core은 *App_Start* 폴더나 *global.asax* 파일을 사용 하지 않으며 web.config 파일은 게시 시간에 추가 *됩니다.* *Startup.cs* 는 *global.asax* 의 대체 항목으로, 프로젝트 루트에 있습니다. 클래스 `Startup` 는 모든 응용 프로그램 시작 작업을 처리 합니다. 자세한 내용은 <xref:fundamentals/startup>를 참조하세요.

ASP.NET Core MVC에서는가에서 <xref:Microsoft.AspNetCore.Builder.MvcApplicationBuilderExtensions.UseMvc*> `Startup.Configure`호출 될 때 특성 라우팅이 기본적으로 포함 됩니다. 다음 `UseMvc` 호출은 *ProductsApp* 프로젝트의 *App_Start/Webapicon.cs* 파일을 대체 합니다.
...
1. 새 프로젝트 이름 (*제품 점수*)과 일치 하도록 복사 된 파일의 네임 스페이스를 변경 합니다. ProductsController.cs 에서도 `using ProductsApp.Models;` 문을 조정 *ProductsController.cs* 합니다.
...
1. `ApiController`을 <xref:Microsoft.AspNetCore.Mvc.ControllerBase>으로 변경합니다. 를 `using Microsoft.AspNetCore.Mvc;` 추가 하 여 `ControllerBase` 참조를 확인 합니다.
1. `using System.Web.Http;`를 삭제합니다.
1. `GetProduct` 작업의 반환 형식을에서 `IHttpActionResult` 로 `ActionResult<Product>`변경 합니다.

`GetProduct` 작업의 `return` 문을 다음과 같이 단순화 합니다.
...
1. 클래스에 `ProductsController` 다음 특성을 표시 합니다.
    ...
    위의 [`[Route]`](xref:Microsoft.AspNetCore.Mvc.RouteAttribute) 특성은 컨트롤러의 특성 라우팅 패턴을 구성 합니다. 특성 [`[ApiController]`](xref:Microsoft.AspNetCore.Mvc.ApiControllerAttribute) 은이 컨트롤러의 모든 동작에 대 한 요구 사항을 라우팅하는 특성을 만듭니다.
...
1. 프로젝트의 호환성 모드를 ASP.NET Core 2.2으로 설정 합니다.
    ...
    * 는 컨트롤러 수준에서 특성 `[ApiController]` 을 사용 하는 데 필요 합니다.
    * ASP.NET Core 2.2에 도입 된 잠재적으로 Opts 수 있습니다.
1. 작업에 대 한 HTTP Get `ProductController` 요청을 사용 하도록 설정 합니다.
    * `GetAllProducts` 작업에 특성을 [`[HttpGet]`](xref:Microsoft.AspNetCore.Mvc.HttpGetAttribute) 적용 합니다.
    * `GetProduct` 작업에 특성을 `[HttpGet("{id}")]` 적용 합니다.

위의 변경 내용 및 사용 되지 않은 `using` 문 제거 후 *ProductsController.cs* 파일은 다음과 같습니다.

[!code-csharp[](webapi/sample/ProductsCore/Controllers/ProductsController.cs)]

마이그레이션된 프로젝트를 실행 하 고로 `/api/products`이동 합니다. 세 제품의 전체 목록이 표시 됩니다. [https://www.microsoft.com]\(`/api/products/1`) 로 이동합니다. 첫 번째 제품이 표시 됩니다.
...
에 `Microsoft.AspNetCore.Mvc.WebApiCompatShim` 포함 된 호환성 기능은 다음과 같습니다.

* 컨트롤러의 `ApiController` 기본 형식을 업데이트할 필요가 없도록 형식을 추가 합니다.
...
* 컨트롤러 작업에서 형식의 `HttpRequestMessage`매개 변수를 사용할 수 있도록 모델 바인딩을 확장 합니다.
* 형식의 `HttpResponseMessage`결과를 반환 하는 작업을 허용 하는 메시지 포맷터를 추가 합니다.
...
* 앱의 추가 DI `IContentNegotiator` (종속성 주입) 컨테이너에 인스턴스를 추가 하 고 [WebApi](https://www.nuget.org/packages/Microsoft.AspNet.WebApi.Client/)의 콘텐츠 협상 관련 형식을 사용 가능 하 게 설정 합니다. 이러한 형식의 예로는 `DefaultContentNegotiator` 및 `MediaTypeFormatter`가 있습니다.
...
1. 에서를 `services.AddMvc().AddWebApiConventions()` `Startup.ConfigureServices`호출 하 여 앱의 DI 컨테이너에 호환성 shim의 서비스를 등록 합니다.
1. 앱 `IApplicationBuilder.UseMvc` 호출의에서를 `IRouteBuilder` 사용 하 `MapWebApiRoute` 여 웹 API 관련 경로를 정의 합니다.
