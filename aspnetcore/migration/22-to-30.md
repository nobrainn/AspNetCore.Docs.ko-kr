...
* `<PropertyGroup>`에서
  ...
  * 요소를 `<AspNetCoreHostingModel>` 제거 합니다. 자세한 내용은이 문서의 [in-process 호스팅 모델](#in-process-hosting-model) 을 참조 하세요.
...
* 또는 `Microsoft.NET.Sdk` `Microsoft.NET.Sdk.Razor` SDK를 대상으로 하는 프로젝트는 명시적 `FrameworkReference` 를 `Microsoft.AspNetCore.App`에 추가 해야 합니다.
...
앱에서 Docker를 사용 하는 경우 ASP.NET Core 3.0를 포함 하는 기본 이미지를 사용 합니다. `docker pull mcr.microsoft.com/dotnet/core/aspnet:3.0`)을 입력합니다.
...
* WebApi NuGet 패키지에 대 한 `System.Net.HttpClient` &ndash; 서식 [Microsoft.AspNet.WebApi.Client](https://www.nuget.org/packages/Microsoft.AspNet.WebApi.Client/) 및 콘텐츠 협상 지원은 `ReadAsAsync` 및 `PostJsonAsync`와 같은 api를 사용 하 여에 `System.Net.HttpClient` 유용한 확장성을 제공 합니다.

* Razor뷰 및 &ndash; 페이지의 Razor 런타임 컴파일에 대 한 런타임 컴파일 지원은 이제 [AspNetCoreRazor의 일부입니다. RuntimeCompilation](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.Razor.RuntimeCompilation).

* MVC `Newtonsoft.Json` (Json.NET)는 &ndash; 현재 ASPNETCORE에 포함 된 `Newtonsoft.Json` mvc 사용에 대 [Microsoft.AspNetCore.Mvc.NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson)한 지원을 지원 합니다.

## <a name="startup-changes"></a>시작 변경

다음 이미지는 ASP.NET Core 2.2 Razor 페이지 웹 앱에서 삭제 된 줄과 변경 된 줄을 보여 줍니다.

![ASP.NET Core 2.2 Razor 웹 앱에서 삭제 되 고 변경 된 줄](22-to-30/_static/startup2.2.png)

위의 그림에서 삭제 된 코드는 빨강으로 표시 됩니다. 삭제 된 코드는 파일을 비교 하기 전에 삭제 된 쿠키 옵션 코드를 표시 하지 않습니다.

다음 이미지는 ASP.NET Core 3.0 Razor 페이지 웹 앱에서 추가 및 변경 된 줄을 보여 줍니다.

![ASP.NET Core 3.0 Razor 웹 앱에서 추가 및 변경 된 줄](22-to-30/_static/startup3.0.png)

위의 그림에서 추가 된 코드는 녹색으로 표시 됩니다. 다음 변경 내용에 대 한 정보:

* `services.AddMvc`에 `services.AddRazorPages`대 한 자세한 내용은이 문서의 [MVC 서비스 등록](#mvc-service-registration) 을 참조 하세요.
* `CompatibilityVersion`을 참조 <xref:mvc/compatibility-version>하십시오.
* `IHostingEnvironment`에 `IWebHostEnvironment`대 한 자세한 내용은 [이 GitHub 공지](https://github.com/dotnet/AspNetCore/issues/7749)를 참조 하세요.
* `app.UseAuthorization`권한 부여 미들웨어를 추가 해야 하는 순서를 보여 주기 위해 템플릿에 추가 되었습니다. 앱에서 권한 부여를 사용 하지 않는 경우에 대 `app.UseAuthorization`한 호출을 안전 하 게 제거할 수 있습니다.
...
RazorMVC에 대 한 UI 구성 요소를 제공 하는 클래스 `AddRazorSupportForMvc` 라이브러리 프로젝트는 프로젝트 파일에서 속성을 설정 해야 합니다.
...
(*Program.cs*)에서 `ConfigureWebHostDefaults` 제공 하는 [웹 호스트 작성기](#hostb) 로 kestrel 구성 마이그레이션:
...
앱이를 사용 `HostBuilder`하 여 호스트를 수동으로 만드는 `UseKestrel` 경우의 `ConfigureWebHostDefaults`웹 호스트 빌더에서를 호출 합니다.
...
* `Microsoft.AspNetCore.Server.Kestrel.Transport.Abstractions.Internal.SchedulingMode`가에서 <xref:Microsoft.AspNetCore.Server.Kestrel.KestrelServerOptions>제거 되었습니다.
...
* `GetDeclaredTrailers`&ndash; 본문 후에 `Trailer` 사용할 트레일러를 나열 하는 요청 헤더를 가져옵니다.
...
`AllowSynchronousIO`,, 등의 동기 i/o api를 사용 하거나 사용 하지 않도록 설정 `Stream.Flush`합니다. `HttpResponse.Body.Write` `HttpRequest.Body.Read` 이러한 Api는 앱 작동 중단에 대 한 스레드 고갈의 소스입니다. 3\.0에서 `AllowSynchronousIO`는 기본적으로 사용하지 않도록 설정됩니다. 자세한 내용은 [Kestrel 문서의 동기 i/o 섹션](/aspnet/core/fundamentals/servers/kestrel?view=aspnetcore-3.0#synchronous-io)을 참조 하세요.

동기 i/o가 필요한 경우에는 사용 중인 서버에서 `AllowSynchronousIO` 옵션을 구성 하 여 사용할 수 있습니다 ( `ConfigureKestrel`예: kestrel을 사용 하는 경우). 서버 (Kestrel, HttpSys, TestServer 등)에는 다른 서버에 영향을 주지 `AllowSynchronousIO` 않는 자체 옵션이 있습니다. 다음 옵션을 `IHttpBodyControlFeature.AllowSynchronousIO` 사용 하 여 요청 별로 모든 서버에 대해 동기 i/o를 사용 하도록 설정할 수 있습니다.
...
Dispose에서 동기 Api를 <xref:System.IO.TextWriter> 호출 하는 다른 스트림이나 구현에 문제가 있는 [Dispose](/dotnet/standard/garbage-collection/implementing-dispose)경우 대신 새 <xref:System.IO.Stream.DisposeAsync*> API를 호출 합니다.
...
ASP.NET Core 2.1에서는 *AspNetCore* 의 내용이 *AspNetCore*로 이동 되었습니다... n a m .dll.. n a m .dll. 이는 특성을 사용 하 `TypeForwardedTo` 는 주요 업데이트가 아닙니다. 3.0의 경우 빈 *AspNetCore* 어셈블리 및 NuGet 패키지는 제거 되었습니다 (예를 들어,
...
ASP.NET Core에 대 한 기본 JSON serializer는 <xref:System.Text.Json>.net Core 3.0의 새로운 버전입니다. 가능 하면 `System.Text.Json` 를 사용 하는 것이 좋습니다. 고성능 이며 추가 라이브러리 종속성이 필요 하지 않습니다. 그러나가 새로운 `System.Text.Json` 이기 때문에 현재 앱에 필요한 기능이 없을 수 있습니다. 자세한 내용은 [newtonsoft.json에서 system.string으로 마이그레이션하는 방법](/dotnet/standard/serialization/system-text-json-migrate-from-newtonsoft-how-to)을 참조 하세요.

### <a name="use-newtonsoftjson-in-an-aspnet-core-30-signalr-project"></a>ASP.NET Core 3.0 SignalR 프로젝트에서 newtonsoft.json 사용
...
* 클라이언트에서 `AddNewtonsoftJsonProtocol` 메서드 호출을 `HubConnectionBuilder` 인스턴스에 연결 합니다.
...
* 서버에서 `AddNewtonsoftJsonProtocol` 메서드 호출을의 `AddSignalR` `Startup.ConfigureServices`메서드 호출에 연결 합니다.
...
* [AspNetCore NewtonsoftJson](https://nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson) 패키지를 설치 합니다.
...
* 를 `Startup.ConfigureServices` 호출 `AddNewtonsoftJson`하도록 업데이트 합니다.
  ...
  * `AddControllers`
  ...
  `Newtonsoft.Json`설정은 다음에 대 한 `AddNewtonsoftJson`호출에서 설정할 수 있습니다.
...
**참고:** `AddNewtonsoftJson` 메서드를 사용할 수 없는 경우 [AspNetCore NewtonsoftJson](https://nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson) 패키지를 설치 했는지 확인 합니다. 일반적인 오류는 [AspNetCore](https://nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson) 패키지 대신 [newtonsoft.json](https://www.nuget.org/packages/Newtonsoft.Json/) 패키지를 설치 하는 것입니다.
...
에서 `IServiceCollection` MVC 시나리오와 관련 된 세 가지 새로운 최상위 확장 메서드를 사용할 수 있습니다. 템플릿은 대신 이러한 새 메서드를 `AddMvc`사용 합니다. 그러나는 `AddMvc` 이전 릴리스에서와 같이 계속 동작 합니다.
...
다음 예에서는 페이지 및 최소 Razor 컨트롤러 지원에 대 한 지원을 추가 합니다. 웹 응용 프로그램 템플릿은 다음 코드를 사용 합니다.
...
* 앱이에서 `Route`구현 `IRouter` 하거나 상속 하는 경우 [DynamicRouteValuesTransformer](https://github.com/dotnet/AspNetCore.Docs/issues/12997) 을 대체로 사용 합니다.
* 앱이 MVC 내에서 `RouteData.Routers` 직접 액세스 하 여 url을 구문 분석 하는 경우 [ParsePathByEndpointName](xref:Microsoft.AspNetCore.Routing.LinkParserEndpointNameAddressExtensions.ParsePathByEndpointName*)를 사용 하 여 바꿀 수 있습니다. 
...
* 앱에서 `AuthorizePage` 또는 `[Authorize]`와 같은 인증/권한 부여 기능을 사용 하는 경우, `UseAuthentication` 및 `UseAuthorization`에 **after**대 `UseRouting` `UseCors` `UseEndpoints`한 호출을 수행 합니다.
...
* 또는 `UseMvc` `UseSignalR` 를로 `UseEndpoints`바꿉니다.
* 앱에서와 `[EnableCors]`같은 [cors](xref:security/cors) 시나리오를 사용 하는 경우 cors를 사용 `UseCors` 하는 다른 미들웨어 앞에에 대 한 호출을 추가 `UseCors` 합니다 `UseAuthentication`( `UseAuthorization`예: `UseEndpoints`, 및 앞에 두기).
* 로 `IHostingEnvironment` `IWebHostEnvironment` 대체 하 고 <xref:Microsoft.Extensions.Hosting?displayProperty=fullName> 네임 `using` 스페이스에 대 한 문을 추가 합니다.
* ( `IApplicationLifetime` <xref:Microsoft.Extensions.Hosting?displayProperty=fullName> 네임 <xref:Microsoft.Extensions.Hosting.IHostApplicationLifetime> 스페이스)로 대체 합니다.
* ( `EnvironmentName` <xref:Microsoft.Extensions.Hosting?displayProperty=fullName> 네임 <xref:Microsoft.Extensions.Hosting.Environments> 스페이스)로 대체 합니다.
...
> [!WARNING]
> 대부분의 `UseAuthentication`앱에서, `UseAuthorization`및 `UseCors` 에 대 한 호출은 `UseRouting` 및 `UseEndpoints` 에 대 한 호출과 적용 사이에 나타나야 합니다.
...
이전에 CORS는 구성 하기가 어려울 수 있습니다. 미들웨어는 일부 사용 사례에서 사용할 수 있도록 제공 되었지만 다른 사용 사례에서 미들웨어 **없이** 사용 하기 위한 MVC 필터입니다. ASP.NET Core 3.0를 사용 하는 경우 CORS가 필요한 모든 앱은 끝점 라우팅과 함께 CORS 미들웨어를 사용 하는 것이 좋습니다. `UseCors`기본 정책으로 제공할 수 있으며, `[EnableCors]` 및 `[DisableCors]` 특성을 사용 하 여 필요한 경우 기본 정책을 재정의할 수 있습니다.
...
ASP.NET Core 3.0 이상에서는 `UseAuthorization` 가에서 `Startup.Configure`호출 되 고 다음 `HomeController` 에 로그인 한 사용자가 필요 합니다.
...
끝점 라우팅을 사용 하는 경우를 구성 `<xref:Microsoft.AspNetCore.Mvc.Authorization.AuthorizeFilter>` 하는 대신 권한 부여 미들웨어에 의존 하는 것이 좋습니다.  앱이 MVC에서을 `AuthorizeFilter` 전역 필터로 사용 하는 경우에 대 `AddAuthorization`한 호출에서 정책을 제공 하도록 코드를 리팩터링 하는 것이 좋습니다.
...
특정 끝점 클래스에 대해 권한 부여를 구성할 수도 있습니다. 다음 코드는 전역 `AuthorizeFilter` 을 구성 하는 MVC 앱을 권한 부여가 필요한 특정 정책을 사용 하는 앱으로 변환 하는 예제입니다.
...
`[Authorize]` 또는를 구성 하거나 `RequireAuthorization` 를 `FallbackPolicy`구성 하 여 모든 끝점에서 권한 부여를 요구 하도록 구성할 수 있습니다. `FallbackPolicy` 는와 다릅니다 `DefaultPolicy`. `DefaultPolicy` 는 `[Authorize]` `RequireAuthorization`또는 `FallbackPolicy` 에서 트리거되고 다른 정책이 설정 되지 않은 경우는 트리거됩니다. `FallbackPolicy`는 처음에 권한 부여 없이 요청을 허용 하도록 구성 됩니다.

다음 예제는 이전 `DefaultPolicy` 예제와 동일 하지만가 지정 된 경우 `FallbackPolicy` `[AllowAnonymous]` 를 제외 하 고를 사용 하 여 항상 모든 끝점에 대 한 인증을 요구 합니다.
...
또한 각 끝점은 권한 부여 요구 사항을 사용자 지정할 수 있습니다. 다음 `UseAuthorization` 예제에서는 `DefaultPolicy`를 사용 하 여 권한 부여를 처리 `/healthz` 하지만 상태 검사 끝점에 `admin` 는 사용자가 필요 합니다.
...
앱에서 사용자 지정 [권한 부여 처리기](xref:security/authorization/policies#authorization-handlers)를 사용 하는 경우 끝점 라우팅은 다른 리소스 형식을 MVC가 아닌 처리기에 전달 합니다. 권한 부여 처리기 컨텍스트 리소스를 형식 <xref:Microsoft.AspNetCore.Mvc.Filters.AuthorizationFilterContext> ( [MVC 필터로 제공](xref:security/authorization/policies#accessing-mvc-request-context-in-handlers)된 리소스 형식)으로 간주 하는 처리기는 형식의 <xref:Microsoft.AspNetCore.Routing.RouteEndpoint> 리소스 (끝점 라우팅을 통해 권한 부여 처리기에 지정 된 리소스 형식)를 처리 하도록 업데이트 해야 합니다.
...
허브 매핑은 SignalR 이제 내 `UseEndpoints`에서 수행 됩니다.

각 허브를에 `MapHub`매핑합니다. 이전 버전과 마찬가지로 각 허브는 명시적으로 나열 됩니다.

다음 예제에서는 `ChatHub` SignalR 허브에 대 한 지원이 추가 되었습니다.
...
라우팅은 이제 MVC 이상에 대 한 지원을 포함 하므로 이러한 메서드가 수행 하는 작업을 명확 하 게 나타내도록 용어가 변경 되었습니다. `MapControllerRoute` / 와 `MapAreaControllerRoute` 같은 / 경로는 추가 된 순서 대로 적용 `MapDefaultControllerRoute` 됩니다. 먼저 특정 경로 (예: 영역에 대 한 경로)를 추가 합니다.
...
* 접미사를 지정 하는 `Async` 데 필요한 링크 생성 예를 들어:
...
* 링크 생성에는 접미사를 `Async` 지정할 필요가 없습니다. 예를 들어:

    ```cshtml
    <a asp-controller="Products" asp-action="List">List</a>
    ```

이 변경 내용은 특성을 [`[ActionName]`](/dotnet/api/microsoft.aspnetcore.mvc.actionnameattribute) 사용 하 여 지정 된 이름에는 영향을 주지 않습니다. 에서 `Startup.ConfigureServices`다음 코드를 사용 하 여 기본 동작을 사용 하지 않도록 설정할 수 있습니다.
...
[이전 라우팅 버전의 차이점](xref:fundamentals/routing#differences-from-earlier-versions-of-routing)에 대 한 설명서에 설명 된 것 처럼 링크 생성에는 몇 가지 차이점이 `Url.Link` 있습니다 (예: 및 유사한 api 사용). 여기에는 다음이 포함됩니다.
...
### <a name="razor-pages"></a>Razor마주보

이제 Razor 매핑 페이지가 내 `UseEndpoints`에서 발생 합니다.

앱 `MapRazorPages` 이 페이지를 사용 Razor 하는 경우를 추가 합니다. 끝점 라우팅은 많은 프레임 워크에 대 한 지원을 포함 Razor 하므로 이제 페이지 추가는 옵트인 (opt in) 됩니다.

다음 `Startup.Configure` 방법에서는 페이지에 `MapRazorPages` 대 한 Razor 지원을 추가 합니다.
...
앱에 레거시 `IRouter` 지원이 필요한 경우에서 `EnableEndpointRouting` `Startup.ConfigureServices`다음 방법 중 하나를 사용 하지 않도록 설정 합니다.
...
끝점 `MapHealthChecks` 라우팅을 사용 하 여 상태 검사를 사용 하려면를 추가 합니다. 메서드 `MapHealthChecks` 는와 유사한 인수를 `UseHealthChecks`허용 합니다. Over `MapHealthChecks` `UseHealthChecks` 를 사용 하는 이점은 권한 부여를 적용 하 고 일치 정책에 대 한 보다 세분화 된 제어를 제공 하는 기능입니다.

다음 예제에서는에서 상태 `MapHealthChecks` 검사 끝점에 대해를 호출 합니다 `/healthz`.
...
에서 `WebHostBuilder` 로 `HostBuilder` 의 가장 중요 한 변경 내용은 [DI (종속성 주입)](xref:fundamentals/dependency-injection)에 있습니다. 를 사용 `HostBuilder`하는 경우 다음을의 생성자에 `Startup`만 삽입할 수 있습니다.
...
AspNetCore의 ASP.NET Core 2.2 및 `AddAuthorization` 하위 메서드 *Microsoft.AspNetCore.Authorization.dll*는 다음과 같습니다.
...
* 페이지를 사용 Razor 하지 않는 앱은 `MapRazorPages`를 호출 해야 합니다. 이 문서의 [ Razor 페이지](#razor-pages) 를 참조 하세요.
...
[Newtonsoft.json에서 지원 되지 않는 기능](/dotnet/standard/serialization/system-text-json-migrate-from-newtonsoft-how-to)을 사용 하는 경우를 다시로 `Newtonsoft.Json`전환할 수 있습니다. 이 문서 앞부분의 [ASP.NET Core 3.0 SignalR 프로젝트에서 newtonsoft.json 사용을](#use-newtonsoftjson-in-an-aspnet-core-30-signalr-project) 참조 하세요.
...
ASP.NET Core 3.0 이전에는 뷰의 런타임 컴파일이 프레임 워크의 암시적 기능 이었습니다. 런타임 컴파일을 통해 뷰의 빌드 시간 컴파일을 보완 합니다. 전체 앱을 다시 빌드하지 않고도 Razor 파일이 수정 될 때 프레임 워크에서 뷰와 페이지 (*cshtml* 파일)를 컴파일할 수 있습니다. 이 기능은 IDE에서 빠른 편집을 수행 하 고 브라우저를 새로 고쳐 변경 내용을 확인 하는 시나리오를 지원 합니다.
...
1. 다음 `Startup.ConfigureServices` 을 호출 `AddRazorRuntimeCompilation`하도록 업데이트:
    ...
    ASP.NET Core Razor 페이지의 경우 다음 코드를 사용 합니다.
...
파일 컴파일에 대 Razor 한 자세한 내용은을 참조 <xref:mvc/views/view-compilation>하십시오.
...
클래스 라이브러리에서 ASP.NET Core Api를 사용 하는 방법에 대 한 <xref:fundamentals/target-aspnetcore>자세한 내용은을 참조 하십시오.
...
[일반 호스트](xref:fundamentals/host/web-host)와 직접 <xref:Microsoft.AspNetCore.TestHost.TestServer> 를 사용 하는 앱의 경우 <xref:Microsoft.AspNetCore.Hosting.IWebHostBuilder> 의 `TestServer` <xref:Microsoft.Extensions.Hosting.GenericHostWebHostBuilderExtensions.ConfigureWebHost%2A>에서를 만듭니다.
