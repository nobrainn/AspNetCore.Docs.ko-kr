...
앱에서 Docker를 사용 하는 경우 ASP.NET Core 3.0를 포함 하는 기본 이미지를 사용 합니다. 정의합니다(예: `docker pull mcr.microsoft.com/dotnet/core/aspnet:3.0`).
...
### <a name="no-locrazor-class-library"></a>Razor클래스 라이브러리
...
ASP.NET Core 3.0에서 ASP.NET Core MVC는 `Async` 컨트롤러 작업 이름에서 접미사를 제거 합니다. 라우팅 및 링크 생성은 모두이 새 기본값의 영향을 받습니다. 예:
...
* 접미사를 지정 하는 데 필요한 링크 생성 `Async` 예:
...
* 링크 생성에는 접미사를 지정할 필요가 없습니다 `Async` . 예:
...
[이전 라우팅 버전의 차이점](xref:fundamentals/routing#differences-from-earlier-versions-of-routing)에 대 한 설명서에 설명 된 것 처럼 링크 생성에는 몇 가지 차이점이 있습니다 ( `Url.Link` 예: 및 유사한 api 사용). 내용은 다음과 같습니다.
