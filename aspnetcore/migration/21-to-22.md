...
* ASP.NET Core Module `<handlers>` (`name="aspNetCore"`)을 추가 하는 항목에서 `modules` 특성 값을에서 `AspNetCoreModule` 로 `AspNetCoreModuleV2`변경 합니다.
* `<aspNetCore>` 요소에서 호스팅 모델 특성 (`hostingModel="InProcess"`)을 추가 합니다.
...
`CreateDefaultBuilder` 앱이를 호출 하지 않고 호스트를 `Program` 클래스에서 수동으로 빌드하는 경우를 <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> 호출 **하기 전에** 를 호출 합니다. `ConfigureKestrel`
