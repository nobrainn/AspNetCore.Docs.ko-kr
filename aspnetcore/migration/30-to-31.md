...
특정 .NET Core SDK 버전을 대상으로 하는 [전역 json](/dotnet/core/tools/global-json) 파일을 사용 하는 경우 설치 된 `version` 3.1 SDK 버전으로 속성을 업데이트 합니다. 예를 들어:
...
프로젝트 파일에서 [대상 프레임 워크 모니커 (TFM)](/dotnet/standard/frameworks) 를로 `netcoreapp3.1`업데이트 합니다.
...
프로젝트 파일에서 각 `Microsoft.AspNetCore.*` 패키지 참조의 `Version` 특성을 3.1.0 이상으로 업데이트 합니다. 예를 들어:
...
Docker를 사용 하는 앱의 경우 ASP.NET Core 3.1를 포함 하는 기본 이미지를 사용 합니다. 예를 들어:
...
HTTP `SameSite` 쿠키에 대 한 특성 구현이 ASP.NET Core 3.0 및 3.1 사이에서 변경 되었습니다. 수행할 작업은 다음 리소스를 참조 하세요.
...
ASP.NET Core 3.1에는 `Component` 태그 도우미가 도입 되었습니다. 태그 도우미는 Blazor 프로젝트의 HTML `RenderComponentAsync<TComponent>` 도우미 메서드를 바꿀 수 있습니다. 예를 들어:
...
자세한 내용은 [구성 요소를 페이지 및 MVC Razor 앱에 통합](/aspnet/core/blazor/integrate-components?view=aspnetcore-3.1)을 참조 하세요.
