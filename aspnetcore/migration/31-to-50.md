...
[전역 json](/dotnet/core/tools/global-json) 파일을 사용 하 여 특정 .NET Core SDK 버전을 대상으로 하는 경우 설치 된 `version` .net 5.0 SDK 버전으로 속성을 업데이트 합니다. 다음은 그 예입니다.
...
프로젝트 파일에서 [대상 프레임 워크 모니커 (TFM)](/dotnet/standard/frameworks) 를로 `net5.0`업데이트 합니다.
...
프로젝트 파일에서 각 `Microsoft.AspNetCore.*` 및 `Microsoft.Extensions.*` 패키지 참조의 `Version` 특성을 5.0.0 이상으로 업데이트 합니다. 다음은 그 예입니다.
...
Docker를 사용 하는 앱의 경우 ASP.NET Core 5.0 런타임을 포함 하는 기본 이미지를 사용 합니다. 다음은 그 예입니다.
