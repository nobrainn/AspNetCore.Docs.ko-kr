---
title: 'ASP.NET Core Razor 구성 요소 만들기 및 사용' author: description: '데이터에 바인딩하고, 이벤트를 처리하고, 구성 요소 수명 주기를 관리하는 방법을 비롯하여 Razor 구성 요소를 만들고 사용하는 방법을 알아봅니다.'
...
다른 네임스페이스에 정의된 구성 요소는 Razor의 [`@using`][2] 지시문을 사용하여 범위로 가져옵니다.

다른 구성 요소 `NavMenu.razor`가 *BlazorSample/Shared/* 폴더에 있는 경우 다음 [`@using`][2] 문을 사용하여 `Index.razor`에서 해당 구성 요소를 사용할 수 있습니다.
