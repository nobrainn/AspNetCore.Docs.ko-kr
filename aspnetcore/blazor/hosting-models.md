---
title: 'ASP.NET Core Blazor 호스팅 모델' author: description: 'Blazor WebAssembly 및 Blazor 서버 호스팅 모델을 이해합니다.'
...
&dagger;‘미리 렌더링’ &ndash; 요청한 Razor 구성 요소가 서버에서 정적 HTML로 컴파일되고 클라이언트로 전송되며, 여기서 사용자에게 렌더링됩니다. 클라이언트와 서버 간에 연결이 설정된 후 구성 요소의 미리 렌더링된 정적 요소가 대화형 요소로 바뀝니다. 미리 렌더링하면 사용자에게 앱의 응답성이 향상된 느낌을 줍니다.
