...
1. **빌드** > **폴더에 게시**를 선택합니다.
1. 게시된 자산을 받을 폴더를 확인하고 **게시**를 선택합니다.
...
`https://www.contoso.com/CoolApp/`이라는 Blazor 앱의 기본 경로에 대한 구성을 제공하기 위해 `<base>` 태그의 `href` 특성은 *Pages/_Host.cshtml* 파일(Blazor 서버) 또는 *wwwroot/index.html* 파일(Blazor WebAssembly)의 상대 루트 경로로 설정됩니다.
...
앱의 기본 경로를 설정하려면 *Pages/_Host.cshtml* 파일(Blazor 서버) 또는 *wwwroot/index.html* 파일(Blazor WebAssembly)의 `<head>` 태그 요소 내에 `<base>` 태그를 업데이트합니다. `href` 특성 값을 `/{RELATIVE URL PATH}/`(뒤에 슬래시가 필요함)로 설정합니다. 여기서 `{RELATIVE URL PATH}`는 앱의 전체 상대 URL 경로입니다.
