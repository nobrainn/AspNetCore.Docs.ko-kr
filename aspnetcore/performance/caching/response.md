...
다음 `Cache-Control` 표에서는 일반적인 지시문을 보여 줍니다.
...
| [Pragma](https://tools.ietf.org/html/rfc7234#section-5.4)  | 동작 설정 `no-cache` 에 대 한 HTTP/1.0 캐시와의 이전 버전과의 호환성을 위해 존재 합니다. `Cache-Control` 헤더가 있으면 `Pragma` 헤더가 무시 됩니다. |
...
[Cache-control 헤더에 대 한 HTTP 1.1 캐싱 지정](https://tools.ietf.org/html/rfc7234#section-5.2) 에는 클라이언트가 보낸 유효한 `Cache-Control` 헤더를 인식 하기 위해 캐시가 필요 합니다. 클라이언트는 `no-cache` 헤더 값을 사용 하 여 요청을 수행 하 고 서버에서 모든 요청에 대 한 새 응답을 생성 하도록 강제할 수 있습니다.

HTTP 캐싱의 목표 `Cache-Control` 를 고려 하는 경우 항상 클라이언트 요청 헤더를 파악 하는 것이 좋습니다. 공식 사양에서 캐싱은 클라이언트, 프록시 및 서버 네트워크에서 요청을 충족 하는 대기 시간 및 네트워크 오버 헤드를 줄이기 위한 것입니다. 원본 서버에서 로드를 제어 하는 방법은 아닙니다.

미들웨어가 공식 캐싱 사양을 준수 하기 때문에 [응답 캐싱 미들웨어](xref:performance/caching/middleware) 를 사용 하는 경우이 캐싱 동작에 대 한 개발자 제어는 없습니다. [미들웨어에 대해 계획 된 향상 된 기능은](https://github.com/dotnet/AspNetCore/issues/2612) 캐시 된 응답을 제공 하도록 결정할 때 요청의 `Cache-Control` 헤더를 무시 하도록 미들웨어를 구성할 수 있는 기회입니다. 계획 된 향상 된 기능을 통해 서버 부하를 보다 효과적으로 제어할 수 있습니다.
...
<xref:Microsoft.AspNetCore.Mvc.CacheProfile.VaryByQueryKeys>지정 된 쿼리 키 목록 값에 따라 저장 된 응답을 변경 합니다. 의 `*` 단일 값이 제공 되 면 미들웨어는 모든 요청 쿼리 문자열 매개 변수의 응답을 변경 합니다.
...
는 <xref:Microsoft.AspNetCore.Mvc.ResponseCacheAttribute> 를 통해 <xref:Microsoft.AspNetCore.Mvc.Filters.IFilterFactory>를 `Microsoft.AspNetCore.Mvc.Internal.ResponseCacheFilter`구성 하 고 만드는 데 사용 됩니다. 는 `ResponseCacheFilter` 응답의 적절 한 HTTP 헤더 및 기능을 업데이트 하는 작업을 수행 합니다. 필터:

* , 및 `Pragma`에 대 한 `Vary`기존 `Cache-Control`헤더를 제거 합니다.
* 에 설정 된 속성을 기반으로 적절 한 헤더를 작성 <xref:Microsoft.AspNetCore.Mvc.ResponseCacheAttribute>합니다.
* 가 설정 된 경우 <xref:Microsoft.AspNetCore.Mvc.CacheProfile.VaryByQueryKeys> 응답 캐싱 HTTP 기능을 업데이트 합니다.

### <a name="vary"></a>날

이 헤더는 속성이 설정 된 경우 <xref:Microsoft.AspNetCore.Mvc.CacheProfile.VaryByHeader> 에만 기록 됩니다. 속성 값으로 `Vary` 설정 된 속성입니다. 다음 샘플에서는 속성을 <xref:Microsoft.AspNetCore.Mvc.CacheProfile.VaryByHeader> 사용 합니다.
...
<xref:Microsoft.AspNetCore.Mvc.CacheProfile.NoStore>는 대부분의 다른 속성을 재정의 합니다. 이 속성이로 `true`설정 되 면 `Cache-Control` 헤더가로 `no-store`설정 됩니다. 가 <xref:Microsoft.AspNetCore.Mvc.CacheProfile.Location> 로 `None`설정 된 경우

* `Cache-Control`가 `no-store,no-cache`로 설정됩니다.
* `Pragma`가 `no-cache`로 설정됩니다.

<xref:Microsoft.AspNetCore.Mvc.CacheProfile.NoStore> 가이 `false` 고 <xref:Microsoft.AspNetCore.Mvc.CacheProfile.Location> 가 `None`, `Cache-Control`및 `Pragma` 이면가로 `no-cache`설정 됩니다.

<xref:Microsoft.AspNetCore.Mvc.CacheProfile.NoStore>는 일반적으로 오류 `true` 페이지에 대해로 설정 됩니다. 샘플 앱의 Cache2 페이지는 클라이언트에 응답을 저장 하지 않도록 지시 하는 응답 헤더를 생성 합니다.
...
캐싱을 사용 하도록 설정 <xref:Microsoft.AspNetCore.Mvc.CacheProfile.Duration> 하려면를 양수 값으로 설정 하 고 <xref:Microsoft.AspNetCore.Mvc.CacheProfile.Location> (기본값) `Any` 또는 `Client`중 하나 여야 합니다. 프레임 워크는 헤더 `Cache-Control` 를 위치 값으로 설정 하 고 그 `max-age` 뒤에 응답의을 설정 합니다.

<xref:Microsoft.AspNetCore.Mvc.CacheProfile.Location>`Any` 및 `Client` 의 옵션은 각각 및 `Cache-Control` `public` `private`의 헤더 값으로 변환 됩니다. [Nostore 및 Location. None](#nostore-and-locationnone) 섹션 <xref:Microsoft.AspNetCore.Mvc.CacheProfile.Location> 에서 설명한 대로을로 `None` 설정 하면 및 `Cache-Control` `Pragma` 헤더가 모두로 `no-cache`설정 됩니다.

`Location.Any`(`Cache-Control` 로 `public`설정)은 *클라이언트 또는 중간 프록시가* [응답 캐싱 미들웨어](xref:performance/caching/middleware)를 포함 하 여 값을 캐시할 수 있음을 나타냅니다.

`Location.Client`(`Cache-Control` 로 `private`설정)는 *클라이언트만* 값을 캐시할 수 있음을 나타냅니다. [응답 캐싱 미들웨어](xref:performance/caching/middleware)를 포함 하 여 값을 캐시 해야 하는 중간 캐시는 없습니다.
...
다음 예제에서는 샘플 앱의 Cache3 페이지 모델 및 기본값 <xref:Microsoft.AspNetCore.Mvc.CacheProfile.Duration> <xref:Microsoft.AspNetCore.Mvc.CacheProfile.Location> 을 설정 하 고 종료 하 여 생성 된 헤더를 보여 줍니다.
...
여러 컨트롤러 작업 특성에 대 한 응답 캐시 설정을 복제 하는 대신에서Razor `Startup.ConfigureServices`MVC/Pages를 설정할 때 캐시 프로필을 옵션으로 구성할 수 있습니다. 참조 된 캐시 프로필에 있는 값은에서 <xref:Microsoft.AspNetCore.Mvc.ResponseCacheAttribute> 기본값으로 사용 되며 특성에 지정 된 모든 속성에 의해 재정의 됩니다.

캐시 프로필을 설정 합니다. 다음 예제에서는 샘플 앱의 `Startup.ConfigureServices`30 초 캐시 프로필을 보여 줍니다.

[!code-csharp[](response/samples/2.x/ResponseCacheSample/Startup.cs?name=snippet1)]

샘플 앱의 Cache4 페이지 모델은 캐시 프로필 `Default30` 을 참조 합니다.

[!code-csharp[](response/samples/2.x/ResponseCacheSample/Pages/Cache4.cshtml.cs?name=snippet)]

는 <xref:Microsoft.AspNetCore.Mvc.ResponseCacheAttribute> 다음에 적용할 수 있습니다.

* Razor페이지 처리기 (클래스) &ndash; 특성은 처리기 메서드에 적용할 수 없습니다.
* MVC 컨트롤러 (클래스)
* MVC 작업 (메서드) &ndash; 메서드 수준 특성은 클래스 수준 특성에 지정 된 설정을 재정의 합니다.

캐시 프로필에의 `Default30` 한 Cache4 페이지 응답에 적용 되는 결과 헤더는 다음과 같습니다.
