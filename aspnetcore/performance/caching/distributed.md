...
* <xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache.Get*><xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache.GetAsync*> &ndash; 는 문자열 키를 수락 하 고 캐시 된 항목을 캐시 `byte[]` 에 있는 경우 배열로 검색 합니다.
* <xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache.Set*><xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache.SetAsync*> &ndash; 는 문자열 키를 사용 하 `byte[]` 여 캐시에 항목 (배열)을 추가 합니다.
* <xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache.Refresh*><xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache.RefreshAsync*> &ndash; 는 해당 키에 따라 캐시에서 항목을 새로 고치고 해당 하는 슬라이딩 만료 시간 제한을 다시 설정 합니다 (있는 경우).
* <xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache.Remove*><xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache.RemoveAsync*> &ndash; 는 문자열 키에 따라 캐시 항목을 제거 합니다.

## <a name="establish-distributed-caching-services"></a>분산 캐싱 서비스 설정

에서 <xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache> `Startup.ConfigureServices`의 구현을 등록 합니다. 이 항목에서 설명 하는 프레임 워크 제공 구현에는 다음이 포함 됩니다.
...
`sql-cache create` 명령을 실행 하 여 SQL Server에서 테이블을 만듭니다. SQL Server`Data Source`인스턴스 (), 데이터베이스`Initial Catalog`(), 스키마 (예 `dbo`:) 및 테이블 이름 (예: `TestCache`)을 제공 합니다.
...
> [!NOTE]
> 앱은가 <xref:Microsoft.Extensions.Caching.SqlServer.SqlServerCache>아닌 인스턴스를 사용 하 여 캐시 <xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache>값을 조작 해야 합니다.
...
> [!NOTE]
> ( <xref:Microsoft.Extensions.Caching.SqlServer.SqlServerCacheOptions.ConnectionString*> <xref:Microsoft.Extensions.Caching.SqlServer.SqlServerCacheOptions.SchemaName*> 및 필요에 따라 및 <xref:Microsoft.Extensions.Caching.SqlServer.SqlServerCacheOptions.TableName*>)는 일반적으로 소스 제어 외부에 저장 됩니다 (예: [암호 관리자](xref:security/app-secrets) 또는 *appsettings*/에서 저장 한 경우 *). 환경}. json* 파일). 연결 문자열에는 원본 제어 시스템에서 유지 되어야 하는 자격 증명이 포함 될 수 있습니다.
...
1. 명령 `redis-server` 프롬프트에서를 실행 합니다.
...
샘플 앱 <xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache> 이 시작 되 면가에 `Startup.Configure`삽입 됩니다. 현재 시간은를 사용 하 여 <xref:Microsoft.Extensions.Hosting.IHostApplicationLifetime> 캐시 됩니다. 자세한 내용은 [제네릭 호스트: IHostApplicationLifetime](xref:fundamentals/host/generic-host#ihostapplicationlifetime)를 참조 하세요.
...
> [!NOTE]
> 인스턴스에 대해 <xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache> Singleton 또는 범위 수명을 사용할 필요는 없습니다 (최소한 기본 제공 구현의 경우).
>
...
NCache 분산 캐시를 사용 하려면 [AspNetCore 메타 패키지](xref:fundamentals/metapackage-app) 를 참조 하 고 패키지 참조를 [NCache](https://www.nuget.org/packages/NCache.Microsoft.Extensions.Caching.OpenSource) 에 추가 합니다.. NCache 패키지는 `Microsoft.AspNetCore.App` 패키지에 포함 되지 않으므로 프로젝트 파일에서 개별적으로 NCache 패키지를 참조 해야 합니다.
...
샘플 앱 <xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache> 이 시작 되 면가에 `Startup.Configure`삽입 됩니다. 현재 시간은를 사용 하 여 <xref:Microsoft.AspNetCore.Hosting.IApplicationLifetime> 캐시 됩니다. 자세한 내용은 [웹 호스트: iapplicationlifetime 인터페이스](xref:fundamentals/host/web-host#iapplicationlifetime-interface)를 참조 하세요.
